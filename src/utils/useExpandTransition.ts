 ...
 *
 * 关键设计：
 * 1. maxH 用 null 表示"不限制"（max-height: none），数字表示受控
 *    - loading 阶段 maxH=null，容器由 Loading 文字自然撑开
 *    - measuring/expanding 阶段 maxH=数字，平滑过渡
 * 2. measuring 分两步设 maxH：
 *    - 第一步 maxH=LOADING_H（从 none→数字，浏览器不过渡，视觉无变化）
 *    - 第二帧 maxH=内容高度（数字→数字，触发平滑过渡）
 *    这样无论内容比 loading 大还是小，都有过渡动画
 *
...
export function useExpandTransition(isLoading: boolean) {
  ...
  // measuring 阶段：内容已渲染但不可见，分两步设 maxH 确保过渡触发
  useEffect(() => {
    if (phase === 'measuring' && ref.current) {
      const measure = () => {
        if (ref.current && phase === 'measuring') {
          ...
          // 第一步：先锁定到 LOADING_H（从 none→数字，浏览器不过渡，视觉无变化）
          setMaxH(LOADING_H)
          // 第二步：下一帧设为内容高度（数字→数字，触发平滑过渡）
